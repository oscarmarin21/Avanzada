...
- **Backend (Java)**  
  ...
  - Config: `backend/src/main/resources/application.yml`. Profiles: `default` (local), `docker` (compose, host `mariadb:3306`), `render` (Render deploy, PostgreSQL via `DATABASE_URL`), `railway` (Railway deploy, MySQL via `SPRING_DATASOURCE_*` or `DATABASE_URL`).  
...
- **Backend (Render, recommended)**: Deploy from repo with root `backend/` (Dockerfile). Use profile `render`; set `SPRING_PROFILES_ACTIVE=render`, `DATABASE_URL` (Render PostgreSQL Internal URL, format `postgres://...`; `RenderDataSourceConfig` converts to JDBC), `JWT_SECRET`, `PORT=9000`. Free tier with PostgreSQL. See README "Deployment".
- **Backend (Railway, alternative)**: Deploy from repo with root or Dockerfile at `backend/`. Use profile `railway`; set `SPRING_PROFILES_ACTIVE=railway`, `SPRING_DATASOURCE_URL` (JDBC URL from Railway MySQL), `JWT_SECRET`. Paid plan required.
- **Frontend (Vercel)**: Root directory `frontend`. Set env `API_URL` to backend API base (e.g. `https://your-app.onrender.com/api`). Build uses `scripts/set-api-url.js` to inject API_URL into `environment.prod.ts`. CORS is enabled on the backend for `*.vercel.app` and `*.onrender.com`.
...
4. **API from frontend**: Dev and Docker use relative `/api/` (proxy/nginx). Production (Vercel) uses `environment.apiUrl` set at build from `API_URL` (e.g. Render or Railway backend + `/api`).  
